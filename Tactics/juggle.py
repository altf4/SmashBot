...
class Juggle(Tactic):
    ...
    def canjuggle(smashbot_state, opponent_state, gamestate, framedata, difficulty):
        ...
        if opponent_state.percent > Infinite.killpercent(
            gamestate.stage, opponent_state.character
        ):
            return False
    ...
    def step(self, gamestate, smashbot_state, opponent_state):
        ...
        if self.framedata.is_roll(opponent_state.character, opponent_state.action):
            ...
            frames_left = (
                self.framedata.last_roll_frame(
        ...
        on_ground = (
            ...
            or opponent_state.position.y < 1
            or opponent_state.action in [Action.TECH_MISS_UP, Action.TECH_MISS_DOWN]
        )
        ...
        if opponent_state.position.y < 5:
            ...
            opponent_state.position.y - side_platform_height
        ) < 5:
        ...
        if self.logger:
            self.logger.log(
                "Notes",
                " Predicted End Position: " + str(end_x) + " " + str(end_y) + " ",
                concat=True,
            )
            self.logger.log("Notes", " on_ground: " + str(on_ground), concat=True)
            self.logger.log(
                "Notes", " frames left: " + str(frames_left) + " ", concat=True
            )
        ...
        if not on_ground:
            ...
            if (
                smashbot_state.action in [Action.TURNING, Action.STANDING]
                and in_range
                and (7 <= frames_left)
            ):
            ...
            for height_level in AirAttack.height_levels():
                ...
                if (commitment - 3 < frames_left) and (
                    abs(smashbot_state.position.x - end_early_x) < 20
                ):
                    ...
                    if -10 < ((height + smashbot_state.position.y) - end_early_y) < 5:
                        if self.logger:
                            ...
                            self.logger.log(
                                "Notes",
                                " height_level: " + str(height_level),
                                concat=True,
                            )
                            self.logger.log(
                                "Notes", " commitment: " + str(commitment), concat=True
                            )
                        self.chain = None
                        self.pickchain(
                            Chains.AirAttack,
            ...
            if self.framedata.is_roll(opponent_state.character, opponent_state.action):
                ...
                if frames_left > 10:
                    ...
                    side_plat_height, side_plat_left, side_plat_right = (
                        melee.side_platform_position(
                            opponent_state.position.x > 0, gamestate
                        )
                    )
                    ...
                    if opponent_state.action in [
                        ...
                        frames_left += 13
                    if (
                        side_plat_height is not None
                        and (frames_left > 25)
                        and abs(side_plat_height - opponent_state.position.y) < 5
                    ):
                        # But only if we're already mostly there
                        smashbot_on_side_plat = (
                            smashbot_state.on_ground
                            and abs(smashbot_state.position.y - side_plat_height) < 5
                        )
                        if (
                            side_plat_left < smashbot_state.position.x < side_plat_right
                            and not smashbot_on_side_plat
                        ):
                            self.chain = None
                            self.pickchain(
                            ...
                            self.pickchain(
                                Chains.DashDance,
                                [(side_plat_left + side_plat_right) / 2],
                            )
                            return
                ...
                if (abs(smashbot_state.position.x - end_x) < 5) and (
                    abs(smashbot_state.position.y - end_y) < 20
                ):
                    # Pivot
                    ...
                    if smashbot_state.action in [Action.TURNING, Action.STANDING]:
                        if 7 <= frames_left <= 9:
                            ...
                            # Can't grab a tech miss. Don't try
                                ...
                                not in [Action.TECH_MISS_UP, Action.TECH_MISS_DOWN]
